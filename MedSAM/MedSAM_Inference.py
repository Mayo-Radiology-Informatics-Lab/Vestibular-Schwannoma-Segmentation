...
# Function to run MedSAM on direct image data
def medsam_inference_direct(image_np, box_np, device, checkpoint_path, output_path=None):
    medsam_model = sam_model_registry["vit_b"](checkpoint=checkpoint_path)
    medsam_model = medsam_model.to(device)
    medsam_model.eval()

    if len(image_np.shape) == 2:  # Grayscale to 3-channel
    ...
    # Convert bounding box to the 1024x1024 scale
