...
modality = "MRI"
#anatomy = "VestSch462"  # anantomy + dataset name
anatomy = 'VestSchPublic'
img_name_suffix = "_0000.nii.gz"
gt_name_suffix = ".nii.gz"
prefix = modality + "_" + anatomy + "_"


nii_path =  # path to the nii images
gt_path = # path to the ground truth
npy_path = "/MedSAM/data/test/npy/" + prefix[:-1]
...
valid_names = []
for name in names:
    image_path = join(nii_path, name.split(gt_name_suffix)[0] + img_name_suffix)
...
print(f"After sanity check, valid number of files: {len(valid_names)}")
